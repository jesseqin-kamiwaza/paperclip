...
import { type TemplateSummary, templatesApi } from "../api/templates";
...
export function OnboardingWizard() {
	...
	// Fetch templates when onboarding opens
	useEffect(() => {
		if (!onboardingOpen) return;
		templatesApi
			.list()
	...
	async function handleTemplateSelect(templateId: string) {
		...
		try {
			...
			// Apply template (creates agents + project + tasks)
			await templatesApi.apply(company.id, templateId);
						...
						<div className="w-full max-w-md mx-auto my-auto px-8 py-12 shrink-0">
										...
										<div className="space-y-3 pt-2">
													...
													>
														...
														<div className="flex-1 min-w-0">
															...
															<div className="flex flex-wrap gap-1 mt-1.5">
																{t.agentNames.map((name) => (
																	<span
																		key={name}
																		className="inline-flex items-center rounded-full px-1.5 py-0.5 text-[10px] bg-muted text-muted-foreground"
																	>
																		{name}
																	</span>
																))}
														...
														{loading && selectedTemplateId === t.id && (
											...
											{!companyName.trim() && (
												<p className="text-[11px] text-muted-foreground text-center">
													Enter a company name above to use a template
												</p>
											)}
										</div>
									)}
							...
							{step === 4 && templateApplied && (
								<div className="space-y-5">
									<div className="flex items-center gap-3 mb-1">
										...
										<div>
											<h3 className="font-medium">Your AI company is ready</h3>
											<p className="text-xs text-muted-foreground">
												Template applied — agents, project, and tasks are set
												up.
											</p>
										</div>
									</div>
							...
							<div className="flex items-center justify-between mt-8">
								...
								<div className="flex items-center gap-2">
									...
									{step === 4 && templateApplied && (
										<Button size="sm" onClick={handleTemplateLaunch}>
