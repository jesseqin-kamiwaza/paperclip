...
export async function startServer(): Promise<StartedServer> {
	...
	async function ensureMigrations(
		...
		if (
			...
			if (repair.repairedMigrations.length > 0) {
				...
				if (state.status === "upToDate")
					return { summary: "already applied", skippedMigrations: [] };
			}
		}
		if (state.status === "upToDate")
			return { summary: "already applied", skippedMigrations: [] };
		if (
			...
			if (!apply) {
				...
				return {
					summary: "pending migrations skipped",
					skippedMigrations: state.pendingMigrations,
				};
			}
			...
			return { summary: "applied (pending migrations)", skippedMigrations: [] };
		}
	...
	if (config.databaseUrl) {
		({ summary: migrationSummary, skippedMigrations } = await ensureMigrations(
			config.databaseUrl,
			"PostgreSQL",
		));
	...
	// Fail fast if PAPERCLIP_MIGRATION_STRICT=true and migrations were skipped.
	// This catches the common dev footgun where PAPERCLIP_MIGRATION_PROMPT=never
	// silently skips pending migrations, leaving the schema stale and causing 500s.
	if (
		skippedMigrations.length > 0 &&
		process.env.PAPERCLIP_MIGRATION_STRICT === "true"
	) {
		throw new Error(
			`Server startup aborted: ${skippedMigrations.length} pending migration(s) not applied: ` +
				`${skippedMigrations.join(", ")}. Run 'pnpm db:migrate' then restart, or unset PAPERCLIP_MIGRATION_STRICT.`,
		);
	}
	...
	const app = await createApp(db as any, {
		...
		pendingMigrations: skippedMigrations,
		betterAuthHandler,
		resolveSession,
	});
		...
		setInterval(() => {
			...
			void heartbeat
				...
				.catch((err) => {
					logger.error(
						{ err },
						"periodic reap of orphaned heartbeat runs failed",
					);
				});
